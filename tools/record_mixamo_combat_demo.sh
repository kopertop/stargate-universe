...
if [[ ! -f models/mixamo_openbot/Swat_rifle_combat.glb && ! -f models/mixamo_openbot/Swat_rifle_idle.glb ]]; then
	echo "ERROR: Mixamo combat pack missing. Rebuild with:" >&2
	echo "  blender -b -P tools/blender_mixamo_rifle_combat.py   # preferred (incoming/ Swat)" >&2
	echo "  blender -b -P tools/blender_mixamo_proxy_combat.py    # cloud fallback (vrm/anim_src)" >&2
	exit 1
fi

echo "=== recording Mixamo combat demo (driver=$RENDER_DRIVER) ==="
"$GODOT_BIN" --path . \
