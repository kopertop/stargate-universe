...
func _handle_movement(delta: float) -> void:
	...
	var want_sprint: bool = Input.is_action_pressed("sprint") and _traversal_mode == TraversalMode.NORMAL and can_sprint and not _mixamo_aiming
	if _mixamo != null and not _mixamo_aiming:
		target_speed *= (1.8 if want_sprint else 1.0)
	elif _mixamo_aiming:
