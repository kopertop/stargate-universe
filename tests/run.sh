...
if [[ "$MODE" == "ftl-cycle" || "$MODE" == "all" ]]; then
	run_script_test "ftl_cycle" "res://tests/smoke/ftl_cycle.gd"
	RC_FTLCYCLE=$?
	RAN_FTLCYCLE=1
fi
...
[[ $RAN_FTLCYCLE -eq 1 ]] && echo "ftl_cycle:           $([[ $RC_FTLCYCLE -eq 0 ]] && echo PASS || echo "FAIL ($RC_FTLCYCLE)")" || echo "ftl_cycle:           SKIPPED"
