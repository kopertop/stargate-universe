...
#
# Usage: tests/run.sh [lint|scene|flow|quest|playthrough|resume|autopilot|questlog|inventory|atmosphere|kino-doors|kino-autoexplore|gamepad|footfall|npc-chat|shaders|ancient-text|discovery-toast|door-plaque|crate|unit-frame|quest-tracker|hud-wow|gate-two-way|equip-mount|equip-assets|char-panel|equip-integration|planet-gen|planet-resources|planet-integration|biome-desert|biome-jungle|biome-toxic|biome-urban|biome-alien-tech|knockout|ftl-loop|music|save|save-integration|elevator-power|bridge-loop|consumption|repair-robot|setdressing|e1-opening|cold-open|away-split|char-gen|vrm|modular|mint-character|mint-loco-combat|mixamo-player|all]
#                                                                          (default: all)
#
...
if [[ "$MODE" == "mixamo-player" || "$MODE" == "all" ]]; then
	run_script_test "mixamo_player_bridge" "res://tests/smoke/mixamo_player_bridge.gd"
	RC_MIXAMO=$?
	RAN_MIXAMO=1
fi
...
[[ $RAN_LINT  -eq 1 || $RAN_MINTLOCO -eq 1 ]] && echo "mint_idle_fingers:   $([[ $RC_MINTIDLE -eq 0 ]] && echo PASS || echo "FAIL ($RC_MINTIDLE)")" || echo "mint_idle_fingers:   SKIPPED"
...
[[ $RAN_MINTCHAR -eq 1 ]] && echo "mint_character:      $([[ $RC_MINTCHAR -eq 0 ]] && echo PASS || echo "FAIL ($RC_MINTCHAR)")" || echo "mint_character:      SKIPPED"
[[ $RAN_MINTLOCO -eq 1 ]] && echo "mint_loco_combat:    $([[ $RC_MINTLOCO -eq 0 ]] && echo PASS || echo "FAIL ($RC_MINTLOCO)")" || echo "mint_loco_combat:    SKIPPED"
