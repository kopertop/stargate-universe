...
| TD-010 | Test | `control_console.gd:59` references bare `GameState`, which emits "Identifier not found" + "Failed to load script" compile-scan noise during the headless `-s` `setdressing` run (per the documented autoload-identifier gotcha). The test still runs 21/21 real assertions and PASSes — cosmetic log noise, not a failure. Fix by reaching the autoload via `get_node_or_null("/root/GameState")` in the scanned path if the noise becomes confusing. | `scripts/control_console.gd` | S | Low | Low | 2026-06-26 | Backlog |
