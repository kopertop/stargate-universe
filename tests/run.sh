...
#
# Usage: tests/run.sh [lint|scene|flow|quest|playthrough|resume|autopilot|questlog|inventory|atmosphere|kino-doors|kino-autoexplore|gamepad|footfall|npc-chat|shaders|ancient-text|discovery-toast|door-plaque|crate|unit-frame|quest-tracker|hud-wow|gate-two-way|equip-mount|equip-assets|char-panel|equip-integration|planet-gen|planet-resources|planet-integration|biome-desert|biome-jungle|biome-toxic|biome-urban|biome-alien-tech|knockout|ftl-loop|music|save|save-integration|elevator-power|bridge-loop|consumption|repair-robot|setdressing|e1-opening|cold-open|away-split|char-gen|vrm|modular|deck|economy|sky|all]
#                                                                          (default: all)
#
...
if [[ "$MODE" == "sky" || "$MODE" == "all" ]]; then
	run_script_test "planet_sky" "res://tests/smoke/planet_sky.gd"
	RC_SKY=$?
	RAN_SKY=1
fi
...
[[ $RAN_SKY -eq 1 ]] && echo "planet_sky:          $([[ $RC_SKY -eq 0 ]] && echo PASS || echo "FAIL ($RC_SKY)")" || echo "planet_sky:          SKIPPED"
