...
func _on_save_pressed() -> void:
	# Manual save uses the same code path as autosave so we exercise one
	# code path. SaveManager.save() is a no-op when current_scene_path is
	# empty, which can't happen here (we already gated _unhandled_input
	# on it) but the defense is cheap.
