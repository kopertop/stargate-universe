...
// ------------------------------------------------------------------
// World Splat Configuration

export type WorldSplatConfig = {
	/**
	 * URL to the gaussian splat file (.spz, .ply, .splat, .ksplat, .sog).
	 * Generated by World Labs Marble API or other 3DGS tools.
	 */
	splatPath: string;
	/**
	 * URL to the collider mesh (.glb) for physics raycasting.
	 * Optional — allows pointer interaction with the splat world.
	 */
	colliderPath?: string;
	/** World position offset for the splat [x, y, z]. Default: [0, 0, 0] */
	position?: [number, number, number];
	/** World scale for the splat. Default: 1.0 */
	scale?: number;
	/**
	 * Whether to create a skybox from the splat's panorama.
	 * Set false when using a separate skybox system.
	 */
	useAsSkybox?: boolean;
};
...
export type GameSceneDefinition = {
	...
	/**
	 * Gaussian splat world configuration.
	 * When set, loads a World Labs gaussian splat as the scene background.
	 * Requires the SparkJS renderer (automatically added to the scene).
	 */
	splatWorld?: WorldSplatConfig;
	/**
