...
// (data/ship_layout.json, data/room_connections.json, web/gate-room/data/chapters.json). Every change rebuilds the ship in
...
export const createLevelEditor = (ctx) => {
	...
	const save = async () => {
		...
		try { await put('/data/ship_layout.json', state.layout); await put('/data/room_connections.json', state.connections); await put('/web/gate-room/data/chapters.json', { chapters: ctx.chapters() }); state.dirty = false; status('saved to repo'); }
